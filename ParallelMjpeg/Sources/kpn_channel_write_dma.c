...
kpn_status_t kpn_channel_write_dma (kpn_channel_t c, void * data, int32_t size)
{
  ...
  watch (kpn_status_t)
  {
	...
	//printf("%d \n", (uint32_t)(void*)(c -> data + c -> position));
	//printf("%d \n", (uint32_t)(void*)((char *)data + position_in_data));
	//printf("Counter : %d \n", counter);

	if(1){
		//printf("Lauching memcpy...\n");
		...
		printf("Lauching mymemcpy...\n");
        	mymemcpy ((void *)(c -> data + c -> position),
            	(void *)((char *)data + position_in_data),
            	bytes_to_transfer);
		}
