...
int32_t decoder (kpn_channel_t c[2])
{
   ...
   */
	...
	while (1) 
  {
    kpn_channel_read_dma (c[0], marker, 2);
		...
		if (marker[0] == M_SMS)
    {
			switch (marker[1])
      {
				case M_SOF0: {
					...
					kpn_channel_read_dma (c[0], & SOF_section, sizeof (SOF_section));
              ...
              (SOF_section . n > 1) ? 's' : ' ');
					...
					kpn_channel_read_dma (c[0], & SOF_component,
            ...
            picture = (uint8_t *) malloc (SOF_section . width
                * SOF_section . height * 2);
				...
				case M_DHT: {
					...
					kpn_channel_read_dma (c[0], & DHT_section, sizeof (DHT_section));
              ...
              & tables[HT_type][HT_index]);
				...
				case M_SOS: {
					...
					kpn_channel_read_dma (c[0], & SOS_section, sizeof (SOS_section));
					cpu_data_is_big_endian(16, SOS_section . length);

					kpn_channel_read_dma (c[0], & SOS_component,
          ...
          kpn_channel_write_dma (c[1], picture, SOF_section . width
              * SOF_section . height * 2);
          ...
          break;
				...
				case M_DQT: {
					...
					kpn_channel_read_dma (c[0], & DQT_section, sizeof (DQT_section));
              ...
              ((DQT_section . pre_quant >> 4) & 0x0f) ? "16 bits" :	"8 bits");
					...
					kpn_channel_read_dma (c[0], DQT_table[QT_index], 64);
				...
				case M_APP0: {
					...
					kpn_channel_read_dma (c[0], & jfif_header, sizeof (jfif_header));
