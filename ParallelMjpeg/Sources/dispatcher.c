...
int32_t dispatcher (kpn_channel_t c[NB_DECODER + 1])
{
  ...
  while (true)
  {
    kpn_channel_read_dma (c[0], & marker0, 1);

    if (marker0 == M_SMS)
    {
      kpn_channel_read_dma (c[0], & marker1, 1);
      ...
      switch (marker1)
      {
        case M_SOI :
          ...
          kpn_channel_write_dma (c[next_decoder + 1], & marker0, 1);
          kpn_channel_write_dma (c[next_decoder + 1], & marker1, 1);
