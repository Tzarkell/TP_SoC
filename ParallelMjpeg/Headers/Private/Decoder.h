...
#include "KahnProcessNetwork.h"
#include <Processor/Processor.h>
#include "kpn_channel_dma.h"
...
static inline void skip_segment (kpn_channel_t channel)
{
  ...
  u;
	...
	kpn_channel_read_dma (channel, & u . size[0], 1);
	kpn_channel_read_dma (channel, & u . size[1], 1);
    ...
    DHT_section_t *DHT_section, huff_table_t * ht)
{
	...
	for (i = 0; i < 16; i++)
  {
		kpn_channel_read_dma (channel, & buffer, 1);
		LeavesN = buffer;
		ht -> ValPtr[i] = LeavesT;
	...
	if (LeavesT > MAX_SIZE((DHT_section -> huff_info & 0x10)))
  {
	...
	kpn_channel_read_dma (channel, ht -> table, max);
    ...
    scan_desc_t *scan_desc, uint8_t number)
{
	...
	for (i = 0; i < number; i++)
  {
    ...
    {
			kpn_channel_read_dma (channel, & wwindow, 1);
			scan_desc -> bit_count = 8;
			if (wwindow == 0xFF) kpn_channel_read_dma (channel, & aux, 1); 
		}
