...
#define CACHE_N_LINE      512

#define N_EXTRA_TARGETS     9
#define N_EXTRA_INITIATORS  3

#define N_GLOBAL_IRQS       3
#define N_LOCAL_IRQS        2
#define PIXEL_WIDTH       256
#define PIXEL_HEIGTH      144
...
int _main(int argc, char *argv[])
{
  ...
  VciSignals<vci_param> signal_aicu_vci_target ("signal_aicu_vci_target");
	...
	*/
	...
	VciSignals<vci_param> signal_framebuffer_vci_target ("signal_framebuffer_vci_target");

	/*
	* Blockdevice signals.
	*/

	sc_signal<bool> signal_blockdevice_it ("signal_blockdevice_it");
	VciSignals<vci_param> signal_blockdevice_vci_target("signal_blockdevice_vci_target");
	VciSignals<vci_param> signal_blockdevice_vci_initiator("signal_blockdevice_vci_initiator");

   /*
    * Dma signals.
    */

    VciSignals<vci_param> signal_dma_vci_target("signal_dma_vci_target");
    VciSignals<vci_param> signal_dma_vci_initiator("signal_dma_vci_initiator");
    sc_signal<bool> signal_dma_it ("signal_dma_it");
  ...
  cctab -> register_non_coherent_initiator (
      maptab . indexForId (IntTab (n_cpus + 1)));
  cctab -> register_non_coherent_initiator (
      maptab . indexForId (IntTab (n_cpus + 2))
      );
  ...
  VciFrameBuffer<vci_param> framebuffer( 
			"framebuffer",
			IntTab (6),
			maptab,
			PIXEL_WIDTH,
	...
	framebuffer . p_clk (signal_clk);
	framebuffer . p_resetn (signal_resetn);
	framebuffer . p_vci (signal_framebuffer_vci_target);
		
  /*
   * BlockDevice
   */

  VciBlockDevice<vci_param> blockdevice( 
			"blockdevice",
			maptab,
			IntTab (n_cpus+1),
			IntTab (7),
			"ice_age_256x144_411.mjpeg");
	
	blockdevice . p_clk (signal_clk);
	blockdevice . p_resetn (signal_resetn);
	blockdevice . p_vci_target (signal_blockdevice_vci_target);
	blockdevice . p_vci_initiator (signal_blockdevice_vci_initiator);
	blockdevice . p_irq (signal_blockdevice_it);
  ...
  dma . p_vci_target (signal_dma_vci_target);
  dma . p_vci_initiator (signal_dma_vci_initiator);
  dma . p_irq (signal_dma_it);
  ...
  aicu . p_irq_in[N_LOCAL_IRQS * n_cpus + 1] (signal_blockdevice_it);
  aicu . p_irq_in[N_LOCAL_IRQS * n_cpus + 2] (signal_dma_it);
  /*
  ...
  vgmn . p_to_initiator[n_cpus+1] (signal_blockdevice_vci_initiator);
  vgmn . p_to_initiator[n_cpus+2] (signal_dma_vci_initiator);
  ...
  vgmn . p_to_target[5] (signal_aicu_vci_target);
	vgmn . p_to_target[6] (signal_framebuffer_vci_target);
	vgmn . p_to_target[7] (signal_blockdevice_vci_target);
    vgmn . p_to_target[8] (signal_dma_vci_target);
